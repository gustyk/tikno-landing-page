...
export function NewsSection() {
  return (
    <section id="berita" className="relative py-20 md:py-28 bg-white overflow-hidden">
      ...
      <FloatingElement className="absolute top-10 right-10 opacity-10" duration={5}>
        <img src="https://kbtkit-alibrahgresik.sch.id/wp-content/uploads/2024/05/pencil-3-1.png" alt="" className="w-14 h-14" />
      </FloatingElement>

      <div className="max-w-7xl mx-auto px-6 md:px-10 lg:px-16 relative z-10">
        <div className="text-center mb-16">
          <FadeIn>
            <span className="inline-block bg-leaf/20 text-leaf-700 px-4 py-1.5 rounded-full text-sm font-semibold mb-4 font-heading animate-float-slow">
              Berita & Inspirasi
            </span>
          </FadeIn>
          <FadeIn delay={0.1}>
            <h2 className="font-heading font-bold text-3xl md:text-4xl text-dark leading-tight mb-3">
              Kabar dan Cerita<br /><span className="text-primary">Kami</span>
            </h2>
          </FadeIn>
          <div className="flex justify-center">
            <CrayonUnderline color="#95E06C" variant="loop" width="160px" />
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item, i) => (
            <FadeIn key={item.title} delay={i * 0.08}>
              <CrayonBorder color={item.crayonColor} variant="rough" className="p-2">
                <a
                  href={item.link}
                  target="_blank"
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="font-heading font-bold text-dark text-sm leading-snug mb-3 group-hover:text-primary transition-colors line-clamp-2">{item.title}</h3>
                    <span className="inline-flex items-center gap-1 text-primary font-semibold text-xs font-heading group-hover:gap-2 transition-all">
                      Baca Selengkapnya <ArrowRight className="w-3.5 h-3.5 group-hover:animate-wiggle" />
                    </span>
                  </div>
                </a>
              </CrayonBorder>
